...
def getsubgraph(layerview, filternode=None, filtervalue=None):
    ...
    filterlist = []
    filterparts = []
    if (filternode is not None) and (filtervalue is not None):
        ...
        for row in drawingelements:
            if (row[glob.elementtype] == 'node') and (not (row[glob.elementsubtype] in layerview)):
                removenodelist = [n for n, v in glob.grh.nodes(data=True) if
                                  (v[settings.label_nodeelement] == row[glob.elementsubtype])]
...
def setgraphattributes(infer=True, contents=None, filename=''):
    if infer:  # infer from graph
        ...
        nodelabels = set()
        ln = nx.get_node_attributes(glob.grh, settings.label_nodeelement)
        nodelabels.update({glob.parent_subtypeelement})
        nodelabels.update(ln.values())
        edgelabels = set()
        le = nx.get_edge_attributes(glob.grh, settings.label_edgeelement)
        edgelabels.update(le.values())
        nodepropdict = dict()
