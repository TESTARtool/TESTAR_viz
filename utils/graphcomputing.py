...
def processgraphmlfile(details=True, advanced=False):
    ...
    if 'All' in settings.centralitynodes:
        ...
        subgraph = getsubgraph(settings.centralitynodes)  # regard only items: NOT in (ABSTRACT,WIDGET or TEST)
    ...
    testsequence_nodes = {n: d for n, d in globals.grh.nodes(data=True) if d[settings.label_nodeelement] == 'TestSequence'}
    ...
    if advanced:
        ...
        initialnodes = [initialnode for nid, daterun, length, initialnode in traces]
        # rename id to nid to prevent shadowing buildin id.
